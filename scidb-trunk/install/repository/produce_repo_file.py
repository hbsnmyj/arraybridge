...
def prepare_package(OS, OSversion):
    ...
    if OS == _OS_CENTOS:
        ...
        replaceVERSION(os.path.join(source_dir, "scidb-repository.spec"), os.path.join(SPECS_dir, "scidb-repository.spec"))
        # Rewrite repo file, substituting scidb _args.version
        replaceVERSION(os.path.join(source_dir, "scidb.repo"), os.path.join(SOURCES_dir, "scidb.repo"))
    elif OS == _OS_UBUNTU:
        source_dir=os.path.join(_MYDIR, "debian_packaging")
        package_dir=os.path.join(_TMPDIR,  "scidb-" + _args.version + "-repository")
        ...
        replaceVERSION(os.path.join(source_dir, "scidb.list"), os.path.join(REPO_dir, "scidb.list"))
...
def create_package(OS):
    ...
    if OS == _OS_CENTOS:
        cmdline = ["rpmbuild",
                   ...
                   "-bb", os.path.join(_TMPDIR, "SPECS", "scidb-repository.spec")]
        if _args.verbose:
        ...
        cmdline = ["dpkg-deb",
                   "--build",
                   os.path.join(_TMPDIR, "scidb-" + _args.version + "-repository")]
        printDebug(cmdline)
...
def copyout_results(OS):
    ...
    if OS == _OS_CENTOS:
        shutil.copy(
            os.path.join(_TMPDIR, "RPMS", "noarch", "scidb-" + _args.version + "-repository-0-1.noarch.rpm"),
            os.path.join(_args.results_dir, "scidb-" + _args.version + "-repository.rpm"))
    elif OS == _OS_UBUNTU:
        shutil.copy(
            os.path.join(_TMPDIR, "scidb-" + _args.version + "-repository.deb"),
            os.path.join(_args.results_dir, "scidb-" + _args.version + "-repository.deb"))
