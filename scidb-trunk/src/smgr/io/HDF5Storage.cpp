...
HDF5File::HDF5File(std::string const &file, bool createFile)
{
    _fileName = file;
    if (createFile) {
        _fileID = H5Fcreate(file.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                            H5P_DEFAULT);
        assert(_fileID >= 0);
    }
}
...
/*
 *  HDF5Dataset Class Implementation
 */
        ...
        : _type(H5T_NO_CLASS)
{
    ...
    _dims.resize(rank);
    H5Sget_simple_extent_dims(_fspaceId, _dims.data(), NULL);
...
int HDF5Dataset::readData(void *data, H5Coordinates const &target_pos, H5Coordinates const &block)
{
    H5Sselect_hyperslab(_fspaceId, H5S_SELECT_SET,
                        target_pos.data(), _stride.data(), _count.data(), block.data());
    hid_t memspace = H5Screate_simple((int) block.size(), block.data(), NULL);
    H5Dread(_datasetId, _type.getHDF5Type(), memspace, _fspaceId, H5P_DEFAULT, data);
    H5Sclose(memspace);
    return 0;
}
...
int HDF5Dataset::readChunk(ConstChunk const &chunk, H5Coordinates const &target_pos, void *data)
{
    auto first = chunk.getFirstPosition(true);
    auto last = chunk.getLastPosition(true);
    H5Coordinates block;
    block.reserve(first.size());
    std::transform(last.begin(), last.end(), first.begin(), std::back_inserter(block),
                   [](hsize_t a, hsize_t b) { return a - b + 1; });
    readData(data, target_pos, block);
    return 0;
}
}
}
